...
- **Global scope**: `/home/user/ajs-website/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/user/ajs-website/.claude/notes/`)
...
2. Glob `/home/user/ajs-website/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/user/ajs-website/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/user/ajs-website/.claude/notes/` — works in any directory
