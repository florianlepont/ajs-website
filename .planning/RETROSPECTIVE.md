...
## Milestone: v1.8 — Cross-linking & Contact CTA

**Shipped:** 2026-08-26 (Phase 24 executed same-day, 45 commits, 2026-08-26 20:14 → 22:24)
**Phases:** 1 (Phase 24) | **Plans:** 5 | **Tasks:** 13

### What Was Built
- A new optional `relatedEdition` reference field on the `gallery` Sanity schema, its own GROQ projection/tri-state sanitizer/pure `getRelatedEditionLink()` helper — mirroring EDN-08's existing forward `relatedGallery` pattern in reverse, closing the missing direction of the v1.3 gallery↔édition cross-link (EDN-12)
- A bordered reverse cross-link rendered at the top of the gallery detail content area, conditional on the field being set, with no broken/empty element when absent
- A universal, unconditional contact CTA (CONT-04) rendered as the last element of both the gallery's and édition's photo sequence — pink hairline, 20px Unbounded text, shared fr/en copy resolved once in `page-models.ts`, styled per sketch-018 Variant B
- Real content published to prove the chain end-to-end: the `silos` gallery now references the `silos` édition, Sanity Studio redeployed so Romane can self-serve the field, and the gallery e2e assertion converted from a conditional contract test to a hard presence + click-through assertion
- UI-03's dual-viewport regression check (390×844 / 1280×900) closed with an explicit human sign-off covering desktop gallery, phone gallery, both édition widths (including the D-09 two-weight judgment call), and a full no-regression sweep

### What Worked
- **The two backlog seeds (SEED-001, SEED-002) that had already survived two consecutive milestone-close carry-forwards (v1.6-finalization, v1.7) as "dormant" were finally scoped and shipped, then marked `status: implemented` in their own seed files rather than acknowledged a third time.** This is the same class of fix v1.7's retrospective flagged for the 6 stale quick-task plans (delete/resolve instead of re-acknowledging) — this time it happened before a third carry-forward, not after.
- **The reverse-of-an-existing-pattern approach (mirror EDN-08's `relatedGallery` shape exactly, just pointed the other way) kept the whole phase to a single day, 5 plans, 45 commits** — schema→data-fetch→render was already a proven shape from Phase 11-13, so no new architectural decisions were needed, only the mirrored implementation plus a D-09 guard proving the existing édition-side related-link styling stayed byte-identical.
- **A worktree-isolated executor caught its own process violation (an accidental `git stash push`) immediately, self-recovered with read-only `git show stash@{...}` instead of `stash pop`, and logged rather than self-resolved the leftover stash entry** — deferred-items.md correctly identifies `git stash drop` as also prohibited for a sub-agent, leaving it for a human/orchestrator with full cross-worktree visibility. The absolute-prohibition list held under an actual live mistake, not just in the abstract.
- **The pre-close audit's 13 open items resolved cleanly into two real categories**: 2 were this milestone's own seeds (genuinely fixed, not deferred) and the other 11 (9 diagnosed debug sessions, 1 stale quick task, 1 permanent `knowledge-base.md` false positive) were the exact same acknowledged set carried since v1.6/v1.7, unchanged — no new debt introduced by this milestone.

### What Was Inefficient
- **`milestone.complete "v1.8"` again failed with "no phases found"** — the fourth consecutive milestone (v1.4 through v1.8, minus v1.5/v1.6 which caught it proactively) to hit some form of this class of bug, because Phase 24's "### Phase 24: ..." heading in ROADMAP.md's Phase Details section had never been wrapped in a versioned "### v1.8 ... (Phase 24)" milestone heading during roadmap creation. Caught and fixed proactively before the CLI call (matching v1.5/v1.6/v1.7's proactive-fix pattern, not v1.3/v1.4's reactive one) — but the underlying gap (roadmapper doesn't emit the wrapping heading for single-phase milestones at creation time) is now five-for-five unfixed at the source.
- **Worktree/install-provisioning gaps (missing `sanity/node_modules`, missing `.env`) surfaced mid-phase for the third-plus time this project has documented** (deferred-items.md items 1 and 3) — both were confirmed environment artifacts, not code defects, by a later closeout agent running in a fresh worktree with a full install. This is the same class of gap `PITFALLS.md`/prior retrospectives have already named; still no standing fix (e.g. a documented worktree-bootstrap step), just repeated case-by-case diagnosis.

### Patterns Established
- A seed carried forward as "dormant" across multiple milestone closes should be resolved to `status: implemented` (with a Resolution section citing the shipping phase) the moment it ships, not left for the audit to keep re-surfacing as open — closing the loop the seed-tracking mechanism itself is for.
- When a milestone closes same-day as its own roadmap/discuss/plan/execute cycle, the ROADMAP.md versioned-heading gap is still worth fixing proactively before calling `milestone.complete`, even though the phase never had a chance to sit "in progress" across a close boundary the way Phase 5/v1.7 did — the bug is about heading presence, not elapsed time.

### Key Lessons
1. **"Acknowledge and defer" is the cheap path for audit items, but genuine resolution (seeds → implemented, stale plans → deleted) should win whenever the underlying work already shipped** — v1.8 is the first milestone to actually take the resolve-not-defer path for items that had already been deferred once before, rather than letting the count grow indefinitely.
2. **The ROADMAP.md missing-versioned-heading bug is now a five-milestone-old, still-unfixed-at-the-source gap** (v1.3, v1.4 hit it reactively; v1.5, v1.6, v1.7, v1.8 now fix it proactively before calling the CLI). Proactive fixing costs one extra edit per close; fixing the roadmapper to emit the heading at phase-creation time would cost zero per close, forever after.

### Cost Observations
- Smallest-scope milestone to date by calendar time: roadmapped and shipped same day (2026-08-26), single phase, 5 plans, 13 tasks, 45 commits, 37 files changed (+4399/-30 lines) — closer in size to v1.1 (1 phase, 1 plan) than to any multi-phase milestone, but with a full wave-gated 5-plan structure (schema → render models → two parallel detail-page plans → a content/UAT checkpoint plan) rather than v1.1's single plan.
- One pre-existing, unrelated flaky test (`accessibility.spec.ts`, homepage accent-palette contrast under parallel test-suite load, per PROJECT.md's Context note) was investigated and confirmed as resource-contention flakiness, not a regression — passes clean in isolation, not carried as a new open item.
...
| v1.8 | — | 1 | Smallest-by-calendar-time milestone (roadmapped and shipped same day). First milestone to resolve previously-deferred backlog seeds (SEED-001, SEED-002) to `status: implemented` at close rather than acknowledging them a third time — breaking the "acknowledge and defer" default the quick-task items had settled into for six straight closes. Fifth milestone in a row to hit the missing-versioned-ROADMAP-heading class of bug (fourth to catch and fix it proactively before calling `milestone.complete`). |
...
| v1.8 | 12/12 verification must-haves; phase-level e2e re-run live at verification: `gallery.spec.ts` 42/42, `edition.spec.ts` 30/30 (both include dual-viewport 390×844/1280×900 assertions); 117/117 unit tests across the 6 files touched by the new data layer (`related-edition`, `related-gallery`, `sanity-validation`, `gallery-query`, `edition-query`, `page-models`, `e2e-content-fragility`); full whole-suite count not separately isolated for v1.8 alone | not separately measured for v1.8 alone | 0 (no new npm dependencies — pure Sanity schema/GROQ/Astro/CSS work) |
...
2. Add the milestone's versioned ROADMAP heading at phase-creation time, not close time, and avoid digit-leading phase-name segments — both v1.3 and v1.4 lost time to the same class of phase-locator tooling gap; v1.5, v1.6, v1.7, and v1.8 all broke the streak by fixing the heading proactively at close time instead, based on the documented lesson — see each milestone's retrospective above. Five milestones in and the fix is still applied per-close, never at the source (the roadmapper itself) — worth fixing there if a sixth occurrence happens.
...
6. **A backlog seed (or any deferred item) should be resolved to "implemented" the moment the work that closes it actually ships — not re-acknowledged at the next audit out of habit.** SEED-001/SEED-002 sat as "dormant" through the v1.6-finalization and v1.7 audits even though nothing about them had changed; v1.8 was scoped specifically to close them, and at close time it would have been easy to just re-list them in the acknowledged table a third time. Checking each open audit item against "did this milestone's own work actually resolve this?" before defaulting to acknowledge-and-carry is what caught it.
