...
**Goal:** Point atelierjacquelinesuzanne.fr at the new Astro/OVH site, replacing the old Myportfolio/Fastly site, without breaking the domain's existing Zimbra mail service — the one requirement (LAUNCH-01) deliberately deferred out of every milestone since v1.0's close on 2026-07-27.

**What shipped** (Phase 5, 6/6 plans):
- `public/contact.php`: a CORS-allowlisted, honeypot-gated, CRLF-hardened PHP `mail()` endpoint replacing the never-provisioned Web3Forms integration — the actual mechanism the site now uses to deliver contact-form submissions
- `.htaccess` hardening (directory-listing lockdown, HTTPS redirect) alongside the pre-existing custom 404
- A manual-dispatch-only, environment-gated `deploy-ovh.yml` GitHub Actions workflow (13 locked CI assertions) pushing the built site to OVH over SFTP
- `scripts/launch-smoke-check.sh` + a six-section, checkbox-able DNS cutover runbook with a named rollback trigger, rehearsed clean against live staging before the real cutover
- The cutover itself: apex/`www` `A` records repointed to OVH hosting, MX/SPF/NS provably byte-identical before and after, TTLs lowered to 60s in advance and restored to 3600s afterward
- Post-cutover verification: automated smoke check (15/15 pass, MX diff clean), plus human-verified real mail delivery (landed in the inbox, not spam), cross-origin submission from the permanently-alive GitHub Pages staging site, and a full browser sweep confirming the old Myportfolio site is gone

**Result:** 1/1 requirement (LAUNCH-01) shipped. Full evidence record at `.planning/phases/05-launch-domain-cutover/05-CUTOVER-LOG.md`.
...
- [x] Visitor can contact Romane via a spam-protected contact form — CONT-01/02, Phase 3 (delivery mechanism switched to OVH PHP `mail()` at Phase 5/v1.7's close, 2026-08-13 — see Key Decisions)
...
v1.5 "Global Improvements & Bug Fixes" shipped and formally closed 2026-08-03 (see `.planning/MILESTONES.md`).
...
- [x] Site is reachable at the existing domain, atelierjacquelinesuzanne.fr, via a rehearsed DNS cutover from the current Myportfolio site — LAUNCH-01, Phase 5. Deliberately deferred since v1.0's close (2026-07-27); executed now: MX/SPF/NS provably unchanged, real mail delivery confirmed to the inbox, old Myportfolio site confirmed gone

**v1.8 (Cross-linking & Contact CTA — Phase 24, shipped 2026-08-26):**
- [x] Visitor viewing a Portfolio gallery detail page with an associated Édition sees a link to it and can navigate there — EDN-12, Phase 24. The reverse of the `relatedGallery` cross-link Phase 13/v1.3 shipped (édition→gallery only); a gallery with no associated édition shows no broken/dead-end element
- [x] Visitor reaching the end of a gallery's or édition's photo sequence sees a contact CTA and can click through to the Contact page — CONT-04, Phase 24. Standing in for a sales flow that doesn't exist yet; universal, no conditional/sold-state logic
- [x] Both features render correctly with no regressions at phone (≤767px) and desktop/tablet (≥768px) widths, confirmed by a human cross-viewport sign-off in addition to automated dual-viewport e2e assertions — UI-03, Phase 24

### Active

**v1.x (add shop, deferred until v1 is live — not yet roadmapped, see items below):**
...
- As of v1.7's close (2026-08-13): atelierjacquelinesuzanne.fr is live, fully replacing the old Myportfolio/Fastly site — the domain now serves the production Astro build over HTTPS (OVH-issued Let's Encrypt cert), plain HTTP redirects to HTTPS, directory listing is locked down, and the site's own custom 404 renders for missing paths. MX/SPF/NS are provably byte-identical to the pre-cutover baseline; the domain's existing Zimbra mailbox continues to receive mail, confirmed by an actual end-to-end contact-form submission landing in the inbox. Contact-form delivery now runs through `public/contact.php` (OVH PHP `mail()`), not Web3Forms — CONT-01's delivery mechanism, open since Phase 3, is finally live. GitHub Pages staging remains permanently alive per D-03 and can still submit the contact form cross-origin to the production endpoint. Two small non-blocking follow-ups closed during wrap-up: the DNS TTLs (lowered to 60s for the cutover window) were restored to 3600s, and the one-time SFTP workaround step added mid-cutover (to clear OVH's pre-provisioned default webroot file) was removed from `deploy-ovh.yml` now that it's served its purpose. The pre-existing `HomeCarousel.astro` touch-handler bug (see v1.6 note above) remains open, unrelated to this milestone.
- As of v1.8's close (2026-08-26): the reverse gallery→édition cross-link (EDN-12) and the universal end-of-sequence contact CTA (CONT-04) are both live, closing backlog seeds SEED-001/SEED-002. Implementation mirrors the shipped `edition.relatedGallery` (EDN-08) pattern in reverse — a new optional `relatedEdition` field on the `gallery` Sanity schema, its own GROQ projection/sanitizer/link helper, and a bordered cross-link rendered at the top of the gallery detail content area; the CTA renders as an unconditional closing element after the photo grid on both gallery and édition detail pages, per the confirmed sketch-018 Variant B design. Real content was published to prove the chain end-to-end: the `silos` gallery now references the `silos` édition (not the `rebut` pairing originally suggested — an equally valid real pairing chosen by the developer), Sanity Studio was redeployed so Romane can self-serve the new field, and the gallery cross-link e2e assertion was converted from a conditional contract test to a hard presence + click-through assertion that fails if the content is ever removed. UI-03's dual-viewport regression check (this project's recurring v1.6 HOME-16-style risk) passed with an explicit human sign-off at both phone and desktop/tablet widths, with no issues reported. One pre-existing, unrelated flaky test (`accessibility.spec.ts`, homepage accent-palette contrast under parallel test-suite load) was confirmed as resource-contention flakiness, not a regression — passes clean in isolation. The v1.x Shop/Checkout/Exhibitions wave remains the only work left, still unscoped in REQUIREMENTS.md's v2 section; no v1.9 milestone has been roadmapped yet.
...
| Phase 5 (Launch & Domain Cutover) executed and closed as its own milestone, v1.7, rather than retroactively folded into the already-shipped-and-tagged v1.0 archive | v1.0 through v1.6 were all already fully shipped, tagged, and archived by the time Phase 5 finally executed (2026-08-11 to 2026-08-13) — v1.0's own archive explicitly excluded Phase 5 from its shipped set. Giving it a new version number reflects it as the last thing actually delivered, without rewriting an already-closed archive | Confirmed — user decision 2026-08-13 |
| DNS TTLs lowered to 60s before the cutover, restored to 3600s (original value) two days after, once the site had been confirmed stable | Standard low-risk-window practice for a DNS cutover — a short TTL keeps rollback cheap during the change, then reverts to normal once nothing needs reverting | Confirmed — restored 2026-08-13, verified via `dig` against both OVH authoritative nameservers |
