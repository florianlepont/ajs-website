...
npm install @astrojs/cloudflare @astrojs/react
npm install @sanity/client @sanity/image-url
npm install stripe

# Dev dependencies
npm install -D wrangler sharp
```
...
**If Romane's monthly sales volume is genuinely tiny (a handful of transactions/month) and the developer wants the absolute least custom code:**
...
**If the developer wants zero third-party SaaS dependencies (not even a free-tier CMS account):**
...
**If shipping/checkout complexity grows significantly post-launch (many SKUs, variants, discount codes, abandoned-cart recovery emails):**
...
**Total baseline recurring cost: 0€/month**, plus per-transaction Stripe fees only when a sale occurs — matching the project's near-zero budget constraint.
