...
- **D-01:** Deploys to the real domain are always an explicit, manual trigger (e.g. `workflow_dispatch` or a manual SFTP push) — never automatic on push to `main`. `main` continues to auto-deploy to GitHub Pages staging as it does today; that pipeline is untouched.
  ...
  - **D-01 refined again (2026-08-11, same day):** The blanket automatic production deploy introduced earlier that day (above) is withdrawn. Production now deploys only from a deliberate `Mettre en production` click in Sanity Studio's editorial dashboard, which publishes a `siteProductionRelease` marker that a dedicated Sanity webhook turns into a `production-deploy-requested` repository_dispatch — a Sanity content publish alone no longer reaches production, it only rebuilds GitHub Pages staging. This restores D-01's original checkpoint intent (an explicit human trigger before the real domain changes) while keeping that trigger reachable by a maintainer with no GitHub access, which is why the unprotected `production-ovh-auto` environment path is retained for this event. See `.planning/quick/260811-w8d-build-the-real-mettre-en-production-feat/`.
