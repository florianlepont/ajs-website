...
- **D-08 (research gate, NOT fully locked):** User's stated preference is to show **"Atelier Jacqueline Suzanne" only as the public identity — no personal name**. However, this is flagged, not locked: French mentions légales legal requirements (LCEN Article 6-III) around identifying the actual site publisher can differ for an individual not yet registered as a business (SIRET pending — see Business Status below) versus a professional/commercial site. The phase researcher MUST verify what LCEN Article 6-III actually requires for Romane's specific situation (individual, SIRET pending, v1 has no checkout/commercial transactions yet) before the planner finalizes the actual identity-disclosure content. If research finds the brand-name-only approach is not compliant, the planner should surface that conflict rather than silently overriding the user's stated preference.

### Business Status Disclosure (not discussed — flagged for research/planning)
- This gray area was intentionally not discussed in this session (user did not select it). Context for whoever resolves it: Romane's SIRET/business registration is still **pending** (per `PROJECT.md` Key Decisions — tracked as a parallel administrative task, not a dev blocker). The mentions légales page's "business status" field must reflect her *actual current* status truthfully — do not assume registration has completed. The planner/researcher should either present options back to the user at plan time, or default to accurate "individual, registration in progress" phrasing and flag it clearly as a placeholder to update once SIRET is confirmed.
