...
- 🚧 **v1.6 Mobile Experience Redesign** — Phases 20-23 (roadmapped 2026-08-03 — mobile-only nav/scroll/lightbox redesign across Homepage, Gallery, and Édition detail pages, plus an About portrait-placement fix; desktop/tablet unchanged throughout) — resolved 2026-08-10, audited 2026-08-11: Phase 20 shipped via the plan/execute cycle (HOME-13 stands; HOME-16 later cancelled — see below); Phase 21 shipped via the plan/execute cycle then its phone-width mechanism was superseded by a direct-commit rebuild (`6c51695`, `MobileHomePrototype.astro`); Phase 22 cancelled (PORT-07/EDN-10/EDN-11 dropped); Phase 23 delivered via the same direct commit. The v1.6 milestone audit (`.planning/v1.6-MILESTONE-AUDIT.md`) found HOME-16 (per-visit random accent color) silently broken by `6c51695`'s renderer swap — cancelled 2026-08-11 rather than fixed, per explicit user decision. No phases remain open in this milestone; not formally archived (no `/gsd-complete-milestone` run)
...
**Requirements**: HOME-13, ~~HOME-16~~ (cancelled 2026-08-11 — see below)
**Success Criteria** (what must be TRUE):
  ...
  3. Revisiting the homepage on a phone across multiple visits shows different accent colors, each one of the existing per-gallery `heroColor` values (no new palette introduced). — Held at Phase 20 close (2026-08-04); silently broken 2026-08-10 by commit `6c51695`'s phone-width renderer swap (`HomeCarousel` → `MobileHomePrototype.astro`, which never received the randomization wiring); found by the v1.6 milestone audit (`.planning/v1.6-MILESTONE-AUDIT.md`) and CANCELLED 2026-08-11 rather than fixed, per explicit user decision.
...
- [x] 21-05-PLAN.md — Wire the pinned, reversible wordmark-to-photo zoom driver and the header hide/fade (HOME-15)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 21-06-PLAN.md — Wire arrival-complete description reveal and live per-slide accent colour, then close the phase gate (HOME-14)
...
- [x] 21-08-PLAN.md — Close UAT gap 3: give deck slides the site's own HOME-09 blur-up placeholder crossfade, plus bounded eager/priority hints and a next-slide warm on arrival

**Wave 7** *(gap closure — blocked on Wave 6 completion)*

- [x] 21-09-PLAN.md — Close UAT gap 4: align the deck's viewport-height strategy with the codebase's real-device-validated `svh` convention, add a paint floor on photo surfaces, and emit a phone-scoped theme-color meta tag
...
- [x] 21-12-PLAN.md — Close round-2 gap 3: collapse the full viewport height of dead zone between zoom completion and the first slide's snap point, and retire the pinned stage once its crossfade job is done, so the cover photo never renders twice

**Round-2 Wave 3** *(blocked on Round-2 Waves 1 and 2)*

- [x] 21-13-PLAN.md — Close round-2 gap 1: replace the two stacked static intro sections with ONE pinned, scroll-scrubbed intro block whose single logomark shrinks continuously as the tagline arrives beneath it (also closes gap 2's intro half)
...
- [x] 21-15-PLAN.md — Close round-2 gap 2's gallery half: replace `applyArrival()`'s symmetric visibility threshold with a two-level reveal/release latch so description text stays readable at a real scroll's resting position, and carry the consolidated real-device gate for all five round-2 gaps
...
**Requirements**: PORT-07, EDN-10, EDN-11 — cancelled, not delivered

### Phase 23: About Portrait Placement & Milestone Regression Close

**Status**: COMPLETE 2026-08-10, delivered via direct commit `6c51695` ("feat: refine mobile gallery journey") outside the plan/execute cycle — no `.planning/phases/23-*/` directory or formal plans were ever created. Accepted complete per explicit user direction (mechanical reconciliation); UI-02's regression sweep was not independently re-verified by this session.
...
**Depends on**: Phase 20, Phase 21 (Phase 22 was cancelled, not a blocker)
**Requirements**: ABOUT-05, UI-02
**Success Criteria** (what must be TRUE):

  1. On a phone-width viewport, the About page's portrait photo appears in a new position relative to the bio text, matching the approved sketch direction, not its pre-milestone placement. — Delivered: `AboutPageBody.astro`'s `.about-page__bio-row` moved from `flex-direction: column-reverse` to `display: block` with the portrait `float: right` (64px) and text wrapping around it at ≤767px; desktop keeps the flex row with explicit `order` swap.
  2. The new portrait placement doesn't crowd, overlap, or push the surrounding text sections awkwardly at common phone widths. — Accepted per user direction, not independently re-verified.
  3. On tablet/desktop viewports, the About page's portrait placement is unchanged from before this milestone. — Accepted per user direction, not independently re-verified.
  4. A combined final pass across Homepage, Gallery detail, Édition detail, and About confirms every desktop/tablet layout, nav, and interaction behavior verified locally in Phases 20-21 still holds unchanged. — Accepted per user direction (UI-02), not independently re-verified; Phase 22's own criteria dropped with its cancellation.

**Plans**: None — delivered directly, no GSD plan/execute cycle
**UI hint**: yes
...
This roadmap covers the **v1 milestone "MVP"** (Phases 1–5 — Phases 1-4/04.1/04.2/04.3 shipped and formally closed 2026-07-12; Phase 5 launch/domain cutover deliberately deferred, not started — see `.planning/MILESTONES.md`), the **v1.1 milestone** (homepage refinements, Phase 6 — HOME-01, HOME-02, HOME-03 — shipped and archived 2026-07-13), the **v1.2 milestone** (homepage polish/pre-launch, Phases 7–10 — HOME-04..HOME-10, I18N-04 — shipped and archived 2026-07-20), and the **v1.3 milestone "Éditions"** (Phases 11–14 — EDN-01..EDN-07, CMS-04 — shipped and archived 2026-07-23; full detail at `.planning/milestones/v1.3-ROADMAP.md`). It also covers the **v1.4 milestone "Editorial Design Consistency"** (Phases 15–16 — ABOUT-03, ABOUT-04, ERR-01 — shipped and formally closed/archived 2026-08-01; full detail at `.planning/milestones/v1.4-ROADMAP.md`). v1.0/v1.1/v1.2 were formally closed retroactively on 2026-07-27 (full accomplishments in `.planning/MILESTONES.md`); their phase detail lives in the `.planning/milestones/v1.3-ROADMAP.md` full-project snapshot since no earlier snapshot existed. It also covers the **v1.5 milestone "Global Improvements & Bug Fixes"** (Phases 17–19 — HOME-11, HOME-12, PORT-04, PORT-05, PORT-06, EDN-09, UI-01, CONT-03 — shipped and formally closed/archived 2026-08-03; full detail at `.planning/milestones/v1.5-ROADMAP.md`). Phase 5 (Launch & Domain Cutover, LAUNCH-01) remains the only open v1 phase, deliberately deferred, not part of any shipped milestone. It also covers the **v1.6 milestone "Mobile Experience Redesign"** (Phases 20–23 — HOME-13, HOME-14, HOME-15, HOME-16, PORT-07, EDN-10, EDN-11, ABOUT-05, UI-02 — roadmapped 2026-08-03, resolved 2026-08-10: 6/9 requirements delivered (Phase 20 fully via the plan/execute cycle; Phase 21 via the plan/execute cycle, phone-width mechanism later superseded by direct commit `6c51695`; Phase 23 via that same direct commit), 3/9 cancelled (Phase 22, PORT-07/EDN-10/EDN-11, explicit user decision); full detail above under Phase Details; not formally archived). The v1.x wave — exhibitions/agenda (EXHB-01, EXHB-02, CMS-02), shop (SHOP-01..04, building on the v1.3 `edition` content model), checkout (CHK-01..05), shipping (SHIP-01, SHIP-02), commerce-specific legal (LEGAL-02, LEGAL-04), the Éditions cross-link differentiator (EDN-08), and related bilingual/CMS extensions (I18N-02b, I18N-03, CMS-03) — is tracked in `.planning/REQUIREMENTS.md`'s v2 section and will get its own roadmap phases once scoped via `/gsd-new-milestone`.
